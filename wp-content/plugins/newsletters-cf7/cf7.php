...
Plugin URI: https://tribulant.com/extensions/view/28/contact-form-7-subscribers
Description: Capture newsletter subscribers into the <a href="https://tribulant.com/plugins/view/1/wordpress-newsletter-plugin">Newsletter plugin</a> from your Contact Form 7 plugin forms.
	...
	if (class_exists('wpMailPlugin')) {
		class wpmlcf7 extends wpMailPlugin {
			...
			function __construct() {
				...
				if (defined('NEWSLETTERS_NAME') && file_exists(dirname(dirname(__FILE__)) . DS . NEWSLETTERS_NAME . DS . 'wp-mailinglist.php')) {
			...
			function check_update($option, $cache = true) {
                ...
                if ( ! isset( $option->response ) || ! is_array( $option->response ) ) {
...
function this_plugin_newsletters_cf7_last() 
{
    // Delay the execution of this function until after the plugins have been loaded
    add_action('init', function () {
        ...
        if ($this_plugin_key !== false) {
            ...
            // Move the plugin to the end of the active plugins list
...
add_action("plugins_loaded", "this_plugin_newsletters_cf7_last");
